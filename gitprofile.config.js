// gitprofile.config.js

const config = {
  github: {
    username: 'khunzinshine', // Your GitHub org/user name. (Required)
    sortBy: 'stars', // stars | updated
    limit: 10, // How many projects to display.
    exclude: {
      forks: false, // Forked projects will not be displayed if set to true.
      projects: [], // These projects will not be displayed. example: ['my-project1', 'my-project2']
    },
  },
  social: {
    linkedin: 'khun-zin-shine',
    twitter: '',
    facebook: '',
    instagram: '',
    dribbble: '',
    behance: '',
    medium: '',
    dev: '',
    website: 'https://khunzinshine.github.io',
    phone: '',
    email: 'khunzinshine@gmail.com',
  },
  skills: [
    'JavaScript',
    'React.js',
    'Node.js',
    'Next.js',
    'MongoDB',
    'SASS',
    'Tailwind',
    'Bootstrap',
    'HTML5',
    'CSS3',
    'Git',
    'Firebase',
    'AWS',
  ],
  experiences: [
    {
      company: 'AYA Innovation Labs (AYA Bank)',
      position: 'Junior React Developer',
      from: 'March 2021',
      to: 'Present',
      description:
        'AYA Innovation Labs is the in-house software development department of the AYA Bank (Digital Banking).',
    },
    {
      company: 'AYA Innovation Labs (AYA Bank)',
      position: 'Quality Assurance Tester',
      from: 'March 2020',
      to: 'Feburary 2021',
      description: '',
    },
    {
      company: 'Acroquest Myanmar Technology Co.,Ltd',
      position: 'Internship',
      from: 'May 2019',
      to: 'July 2019',
      description:
        'Acroquest Myanmar is subsidiary corporation of Acroquest Technology Co.,Ltd (Japan).',
    },
  ],
  education: [
    {
      institution: 'University of Computer Studies, Taunggyi, Myanmar',
      degree: 'Bachelor of Computer Science',
      from: '2014',
      to: '2019',
    },
  ],
  // Display blog posts from your medium or dev account. (Optional)
  // blog: {
  //   source: 'dev', // medium | dev
  //   username: 'arifszn',
  //   limit: 3, // How many posts to display. Max is 10.
  // },
  googleAnalytics: {
    // GA3 tracking id/GA4 tag id UA-XXXXXXXXX-X | G-XXXXXXXXXX
    id: '', // Please remove this and use your own tag id or keep it empty
  },
  hotjar: {
    id: '', //  Please remove this and use your own id or keep it empty
    snippetVersion: 6,
  },
  themeConfig: {
    defaultTheme: 'corporate',

    // Hides the switch in the navbar
    // Useful if you want to support a single color mode
    disableSwitch: false,

    // Should use the prefers-color-scheme media-query,
    // using user system preferences, instead of the hardcoded defaultTheme
    respectPrefersColorScheme: false,

    // Available themes. To remove any theme, exclude from here.
    themes: [
      'light',
      'dark',
      'cupcake',
      'bumblebee',
      'emerald',
      'corporate',
      'synthwave',
      'retro',
      'cyberpunk',
      'valentine',
      'halloween',
      'garden',
      'forest',
      'aqua',
      'lofi',
      'pastel',
      'fantasy',
      'wireframe',
      'black',
      'luxury',
      'dracula',
      'cmyk',
      'autumn',
      'business',
      'acid',
      'lemonade',
      'night',
      'coffee',
      'winter',
      'procyon',
    ],

    // Custom theme
    customTheme: {
      primary: '#fc055b',
      secondary: '#219aaf',
      accent: '#e8d03a',
      neutral: '#2A2730',
      'base-100': '#E3E3ED',
      '--rounded-box': '3rem',
      '--rounded-btn': '3rem',
    },
  },
};

export default config;
